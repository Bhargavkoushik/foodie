...
const SearchBar = ({
  ...
  // Derive real suggestions dynamically from available food items and categories
  const realFoodSuggestions = useMemo(() => {
    if (!food_list || !Array.isArray(food_list) || food_list.length === 0) {
      return [];
    }
    const names = food_list.map((item) => item.name).filter(Boolean);
    const categories = food_list.map((item) => item.category).filter(Boolean);
    return Array.from(new Set([...names, ...categories]));
  }, [food_list]);

  const allSuggestions = useMemo(
    () => (suggestions.length > 0 ? suggestions : realFoodSuggestions),
    [suggestions, realFoodSuggestions]
  );
  ...
  const allPopularSearches = useMemo(
    () => popularSearches.length > 0 ? popularSearches : realFoodSuggestions.slice(0, 5),
    [popularSearches, realFoodSuggestions]
  );
