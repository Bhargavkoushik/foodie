...
const LoginPopup = ({ setShowLogin, setIsLoggedIn }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        setErrorMessage("");
        toast.success(data.message || (currState === "Sign Up" ? "Account created successfully!" : "Logged in successfully!"));
        ...
        // Dispatch storage event so navbar and other components update immediately
        ...
        const msg = data.message || `${currState} failed. Please try again.`;
        setErrorMessage(msg);
        toast.error(msg);
      }
    } catch (err) {
    ...
    <div className='LoginPopup'>
      ...
      <form ref={popupRef} className="login-popup-container" onSubmit={handleSubmit}>
        <div className="login-popup-title">
          ...
          <img onClick={() => setShowLogin(false)} src={assets.cross_icon} alt="close modal" title="Close" />
        </div>
            ...
            <>
              ...
              {currState === "Login" && (
                <div className="password-input-container">
                  ...
                  >
                    {showLoginPassword ? <EyeOffIcon size={20} color="white" /> : <EyeIcon size={20} color="white" />}
                  </span>
                </div>
              )}
        ...
        {!forgotFlow && (
          currState === "Login" ? (
            <p style={{ color: '#ddd' }}>Create a new account? <span onClick={() => handleStateChange("Sign Up")}>Click Here</span></p>
          ) : (
            <p style={{ color: '#ddd' }}>Already have an account? <span onClick={() => handleStateChange("Login")}>Login Here</span></p>
          )
        )}
      </form>
    </div>
